    ...
    with open(csv_file, mode="r", encoding="utf8") as csv_file:
        ...
        for row in reader:
            if float(row["profit"]) > 0 and float(row["price"]) > 0:
                ...
                cleaned_dict[row["name"]].append(round(float(row["price"]) * 100))
                cleaned_dict[row["name"]].append(round(float(row["price"]) 
                                                    * float(row["profit"])))
        return(cleaned_dict)
...
def optimised_tester(dict):
    ...
    print(f"L'éxécution du script optimisé à prit environ {elapsed_time} secondes.\n")
